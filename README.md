...
- 如果使用Docker部署：Docker和Docker Compose
...
项目支持通过环境变量配置，可以在`.env`文件中设置或在Docker Compose中配置：

- `MONGO_URI`: MongoDB连接URI，默认为`mongodb://localhost:27017/`
- `HOST`: API服务器绑定地址，默认为`0.0.0.0`
- `PORT`: API服务器端口，默认为`8080`
- `HOST_URL`: 访问地址，默认为`localhost`
- `API_PORT`: API服务器端口，默认为`8080`
- `HTTP_PORT`: HTTP服务器端口，默认为`8000`

## Docker镜像

本项目的Docker镜像已发布到Docker Hub：

```bash
docker pull xszhu002/ai_paintting:latest
```
...
3. **端口冲突**：如果8080或8000端口被占用，请修改对应环境变量

4. **检查日志**：查看命令行输出或Docker日志，寻找可能的错误信息
   ```bash
